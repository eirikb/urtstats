...
class KillEventTests extends GrailsUnitTestCase {
    ...
    void testGetTotalRatio() {
        def killer = new Player(urtID:0, guid:"0", ip:"1", nick:"1", colorNick:"1").save()
        killer.addGuid()
        def killed = new Player(urtID:1, guid:"1", ip:"1", nick:"1", colorNick:"1").save()
        killed.addGuid()
        assertEquals 1, PlayerTool.getTotalRatio(killer)
        new Kill(killer:killer, killed:killed, deathCause:DeathCause.findByUrtID(14), firendyfire:false).save()
        assertEquals 2, PlayerTool.getTotalRatio(killer)
        assertEquals 0.5, PlayerTool.getTotalRatio(killed)
        new Kill(killer:killed, killed:killer, deathCause:DeathCause.findByUrtID(14), firendyfire:false).save()
        assertEquals 1, PlayerTool.getTotalRatio(killer)
        assertEquals 1, PlayerTool.getTotalRatio(killer)
        new Kill(killer:killer, killed:killed, deathCause:DeathCause.findByUrtID(14), firendyfire:false).save()
        new Kill(killer:killer, killed:killed, deathCause:DeathCause.findByUrtID(14), firendyfire:false).save()
